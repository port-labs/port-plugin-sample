...
│   ├── App.tsx              # Main app: usePortPluginData, applyThemeCss, color-scheme shell
│   ├── portHostTheme.ts     # theme.mode → color-scheme / rdg-dark | rdg-light (local dev defaults to light)
│   ├── App.css
...
The typed data table (`TypedDataTable`) reads **`theme.mode`** from **`usePortPluginData()`** and applies **`rdg-dark` / `rdg-light`** on the grid root so `@port-labs/port-react-data-grid` uses the correct **`color-scheme`** (its internal `light-dark()` tokens track the host). The app shell sets the same **`colorScheme`** on `.plugin-container` so native controls and scrollbars match. When `theme` is not yet available (e.g. local `yarn dev` without `PLUGIN_DATA`), the default is **light**; embedded in Port, **`theme.mode`** from the host selects light or dark.
